...
| 2b-cal | **Nothing extra** — availability sync reuses the calendar scope you already granted (the write scope covers freebusy; asking for `calendar.readonly` *alongside* it is refused with `invalid_scope` when the refresh token was consented for calendar only) | (no new var) | Without any Google credentials at all, *Sync availability* reports `simulated` with zero blocks and Stage A uses declared working hours only. |
