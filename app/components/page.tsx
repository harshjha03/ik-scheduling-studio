...
export default function Page() {
  ...
  const rosterFor = useCallback((w: WeekKey, off: Record<string, boolean> = unavailable,
                                 edits: Record<string, Partial<SME>> = smeEdits): SME[] => (
    [...smeData[w], ...extraSmes].map((s) => {
      const merged = edits[s.id] ? { ...s, ...edits[s.id] } : s;
    ...
    (async () => {
      ...
      try {
        ...
        if (saved?.draft?.length && saved.stats) {
          ...
          // "the draft you saved" claimed more than the payload holds: the week's rows and overrides
          // are restored, but approvals, dismissed work items and leave are page state and are not.
          say(`Restored the week saved at ${new Date(saved.updated_at).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })} — sign-offs and dismissed items start fresh.`);
          return;
        }
  ...
  // Live rows only: a cancelled or merged class is not an unstaffed one. Counting it here blocked
  // the approve and publish guards below on a class nobody can ever staff.
  const openRows = useMemo(() => liveRows(rows), [rows]);
  const unfilledCount = openRows.filter((r) => !r.sme_id).length;
  const conflictCount = openRows.filter((r) => r.flags.some((f) => f.code === "HARD_CONFLICT")).length;
  ...
  const assign = (row: DraftRow, smeId: string, name: string, blocked: boolean, quiet = false) => {
    ...
    // Named, because the same patch is applied twice: once to React state for the screen, and once
    // to the copy that goes to the database. Inlining it here was the reason the two drifted.
    const patch = (x: DraftRow): DraftRow => ({
      ...x,
    ...
    patchRow(week, row.session_id, patch);
    ...
    // REVIEW S2. A re-run persisted the week; a manual override did not, so the same kind of edit
    // survived a refresh or vanished depending on which path made it. Leave requests and dismissed
    // work items are review scratch and stay in the page — draft rows never were. The flat `flags`
    // list is rebuilt from the patched rows so it cannot drift from what the rows now say.
    const weekRun = runs[week];
    if (weekRun) {
      const draft = weekRun.draft.map((x) => (x.session_id === row.session_id ? patch(x) : x));
      if (week === "next") nextRef.current = draft;
      persist(save(WEEKS[week].iso, draft, {
        stats: weekRun.stats,
        flags: draft.flatMap((r) => r.flags).sort((a, b) => a.priority - b.priority
          || a.session_id.localeCompare(b.session_id)),
        // the draft week is un-published by this edit; the live week stays published and is amended
        published: WEEKS[week].locked ? wasPublished : false,
        provenance, history: historyRecords,
      }));
    }
  ...
  const saveProfile = async () => {
    ...
    const before = rosterFor("next").find((s) => s.id === prof.id);
    const patch: Partial<SME> = {
      ...smeEdits[prof.id], name: prof.name.trim(), email: prof.email.trim(), phone: prof.phone.trim(),
      city: prof.city.trim(), level: prof.level, preferred: pref,
      training_level: META.levels.indexOf(prof.level) + 1,
    };
    const nextEdits = { ...smeEdits, [prof.id]: patch };
    setSmeEdits(nextEdits);
    // An e-mail is who a calendar invite is addressed to, and a training level decides who may teach.
    // Neither is review scratch, so the edited roster goes to the same `dataset` rows an import writes
    // and the boot effect already reads back. Both weeks: the live week addresses invites too.
    const bake = (list: SME[]) => list.map((s) => (s.id === prof.id ? { ...s, ...patch } : s));
    persist(putData("smes", bake([...smeData.next, ...extraSmes]), "profile edit"));
    persist(putData("smes_current", bake(smeData.current), "profile edit"));
    if (prof.id === META.me) setPreferred(pref);
    setSheet(null);
    setProf(null);

    // A training level decides Stage A eligibility, so changing it makes the draft on screen stale
    // until something else happens to re-run. Re-draft here instead of waiting for that.
    const levelMoved = !!before && before.training_level !== patch.training_level;
    // Name, e-mail and phone are what a calendar invite, a digest and an SMS are addressed with. On a
    // week people already hold, changing one means what they hold is now wrong — the same staleness a
    // teacher swap creates, so it takes the same route: the draft week un-publishes, the live week
    // stays published and is marked amended so only this teacher's classes are re-sent.
    const contactMoved = !!before && (before.email !== patch.email || before.phone !== patch.phone
      || before.name !== patch.name);

    if (levelMoved) await runWeek(week, { smes: rosterFor(week, unavailable, nextEdits), quiet: true });

    if (contactMoved) {
      const theirs = new Set<string>();
      (["current", "next"] as WeekKey[]).forEach((w) => {
        if (!published[w]) return;
        const mine = (runs[w]?.draft ?? []).filter((r) => r.sme_id === prof.id && isLive(r));
        if (!mine.length) return;
        if (WEEKS[w].locked) setAmended((a) => ({ ...a, [w]: true }));
        else setPublished((p) => ({ ...p, [w]: false }));
        if (w === week) mine.forEach((r) => theirs.add(r.session_id));
      });
      if (theirs.size) setChanged((c) => new Set([...c, ...theirs]));
      say(theirs.size
        ? `${prof.name.trim()}’s details updated — ${theirs.size} class(es) they hold were already sent, so re-publish to update them.`
        : `${prof.name.trim()}’s profile updated.`);
      return;
    }
    say(levelMoved
      ? `${prof.name.trim()}’s profile updated — the draft re-ran against their new level.`
      : `${prof.name.trim()}’s profile updated.`);
  };
      ...
      return (
        <Sheet
          ...
          footer={[
            ...
            ...(emailOk && nameOk ? [{ label: "Save changes", kind: "primary" as const, onClick: () => void saveProfile() }] : []),
          ]}
